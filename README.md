...
- Settings has an *Only the device voice* switch (`speechless:device-voice`
  in localStorage). With it on, Play never contacts Google and goes straight
...
Slovak; `main.js` links to it from Settings with the interface language in
the hash, and `style.css` shows only that section where `:has()` is
...
`index.html` and `privacy.html` carry Open Graph and Twitter card tags, with
an absolute `og:image` URL because crawlers do not resolve relative ones.
The picture is `og-image.png`, 1200×630, generated by `make-og-image.mjs` the
same way as the icons:

```sh
npm i playwright-core
node make-og-image.mjs
```
...
the references in `index.html` and `privacy.html`, and stamps those names into
the service worker.
...
The service worker precaches those hashed files and `privacy.html`, which is
why the privacy page is part of the build id too: without that a reworded page
would never reach an installed app that stays offline. `og-image.png` is copied
